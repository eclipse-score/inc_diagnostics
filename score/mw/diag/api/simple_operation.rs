...
use operation::{
    ExecuteArguments, ExecutionControl, ExecutionEventKind, ExecutionHandle, ExecutionId, ExecutionStatus,
...
impl operation::Operation for SimpleOperationAdapter {
    fn execute(
        ...
        let mut data = self.data.lock().map_err(|_| common::Error::mutex_poisoned())?;
        if data.active_exec_id.is_some() {
            return Err(common::Error::from_error(common::sovd::GenericError::from_code(
                                    ...
                                    .and_then(|mut data| data.op_instance.stop(exec_event.args))
                                {
                                    Ok(Some(result)) => {
                                        ...
                                        exec_event.status_reporter.put(
                                            exec_status.clone(),
                                            ExecutionStatusDetails::default().with_reply_data(result),
                                        );
                                    },
                                    Ok(None) => {
                                        exec_status = ExecutionStatus::Stopped;
                                    ...
                                    Err(err) => {
                                        ...
                                        exec_event.status_reporter.put(
                                            exec_status.clone(),
                                            ExecutionStatusDetails::default().with_exec_errors(exec_errors.clone()),
                                        );
                                    },
                                }
                            },

                            _ => exec_status = ExecutionStatus::UnsupportedCapability(last_exec_event_kind.to_string()),
                        }
                    },
                }
            }
...
mod tests {
    ...
    struct MockSimpleOp {
        ...
        stop_fn: Box<dyn FnMut(Option<ExecuteArguments>) -> DiagResult<Option<DiagnosticReply>> + Send>,
    ...
    impl MockSimpleOp {
        ...
        where
            ...
            T: FnMut(Option<ExecuteArguments>) -> DiagResult<Option<DiagnosticReply>> + Send + 'static,
        {
            Self {
    ...
    fn exec_control_from_kinds(kinds: Vec<ExecutionEventKind>, exec_id: &str) -> Box<dyn ExecutionControl> {
        Box::new(MockExecControl::from_kinds(kinds, exec_id.to_string()))
    }

    /// Helper: create an `ExecutionControl` from a list of events.
    fn exec_control_from_events(events: Vec<ExecutionEvent>, exec_id: &str) -> Box<dyn ExecutionControl> {
    ...
    fn adapter_from_creates_instance() {
        let op = MockSimpleOp::new(
            |_| Ok(ExecutionHandle::from_closure(|| Ok(DiagnosticReply::default()))),
            |_| Ok(None),
        );
    ...
    async fn adapter_execute_resolves_op_future() {
        ...
        let handle = operation::Operation::execute(&mut adapter, default_exec_args(), ctrl).unwrap();
    ...
    async fn adapter_reports_status_while_running() {
        let op = MockSimpleOp::new(
            |_| Ok(ExecutionHandle::from_future(Box::pin(futures::future::pending()))),
            |_| Ok(None),
        );
        let mut adapter = SimpleOperationAdapter::new(op);

        let captured_status: Arc<Mutex<Option<ExecutionStatus>>> = Arc::new(Mutex::new(None));
        let captured_details: Arc<Mutex<Option<ExecutionStatusDetails>>> = Arc::new(Mutex::new(None));
        let cs = captured_status.clone();
        let cd = captured_details.clone();

        let report_event =
            ExecutionEvent::new(ExecutionEventKind::ReportStatus).with_status_reporter(move |status, details| {
                *cs.lock().unwrap() = Some(status);
                *cd.lock().unwrap() = Some(details);
            });
        let events = vec![report_event, ExecutionEvent::new(ExecutionEventKind::ControlGone)];
        let ctrl = exec_control_from_events(events, "exec-id");
        let handle = operation::Operation::execute(&mut adapter, default_exec_args(), ctrl).unwrap();
        let _ = handle.future.await;

        let status = captured_status.lock().unwrap().take().expect("no status was reported");
    ...
    async fn adapter_reports_status_with_completion_percentage() {
        ...
        let report_event =
            ExecutionEvent::new(ExecutionEventKind::ReportStatus).with_status_reporter(move |_status, details| {
                *cd.lock().unwrap() = Some(details);
            });
        let events = vec![report_event, ExecutionEvent::new(ExecutionEventKind::ControlGone)];
        let ctrl = exec_control_from_events(events, "exec-id");
    ...
    async fn adapter_stop_returns_reply() {
        let op = MockSimpleOp::new(
            ...
            |_| {
                Ok(Some(DiagnosticReply {
                    message_payload: Some(ReplyMessagePayload::from_string("stopped successfully".to_string())),
                    additional_attrs: None,
                }))
            },
        );
        ...
        let stop_event = ExecutionEvent::new(ExecutionEventKind::Stop).with_status_reporter(move |status, details| {
            *cs.lock().unwrap() = Some(status);
            *cd.lock().unwrap() = Some(details);
        });
        let events = vec![stop_event, ExecutionEvent::new(ExecutionEventKind::ControlGone)];
        let ctrl = exec_control_from_events(events, "exec-id");
        let handle = operation::Operation::execute(&mut adapter, default_exec_args(), ctrl).unwrap();
        let _ = handle.future.await;
        ...
        assert_eq!(
            ...
            Some(ReplyMessagePayload::from_string("stopped successfully".to_string()))
        );
        assert!(details.exec_errors.is_none());
    }
    ...
    async fn adapter_stop_returns_error() {
        ...
        let stop_event = ExecutionEvent::new(ExecutionEventKind::Stop).with_status_reporter(move |_status, details| {
            *cd.lock().unwrap() = Some(details);
        });
        let events = vec![stop_event, ExecutionEvent::new(ExecutionEventKind::ControlGone)];
        let ctrl = exec_control_from_events(events, "exec-id");
    ...
    async fn adapter_unsupported_event_kind_interrupt() {
        ...
        let report_event =
            ExecutionEvent::new(ExecutionEventKind::ReportStatus).with_status_reporter(move |status, _details| {
                *cs.lock().unwrap() = Some(status);
            });
        let events = vec![
        ...
        let status = captured_status.lock().unwrap().take().expect("no status was reported");
        assert_eq!(status, ExecutionStatus::UnsupportedCapability("freeze".to_string()));
    }
    ...
    async fn adapter_unsupported_event_kind_resume() {
        ...
        let status = captured_status.lock().unwrap().take().expect("no status was reported");
        assert_eq!(status, ExecutionStatus::UnsupportedCapability("execute".to_string()));
    }
    ...
    async fn adapter_unsupported_event_kind_custom_capability() {
        ...
        let events = vec![
            ExecutionEvent::new(ExecutionEventKind::HandleCustomCapability("custom".to_string())),
            report_event,
            ExecutionEvent::new(ExecutionEventKind::ControlGone),
        ];
        let ctrl = exec_control_from_events(events, "exec-id");
        ...
        let status = captured_status.lock().unwrap().take().expect("no status was reported");
        assert_eq!(status, ExecutionStatus::UnsupportedCapability("custom".to_string()));
    }
    ...
    async fn adapter_rejects_concurrent_execution() {
        ...
        let _handle1 = operation::Operation::execute(&mut adapter, default_exec_args(), ctrl1).unwrap();

        // Second execution — should be rejected because active_exec_id is set.
        let ctrl2 = exec_control_from_kinds(vec![ExecutionEventKind::ControlGone], "exec-id-second");
    ...
    async fn adapter_report_status_with_accumulated_errors() {
        ...
        let details = captured_details.lock().unwrap().take().expect("details were reported");
        assert!(details.exec_errors.is_some());
        let exec_errors = details.exec_errors.unwrap();
        assert_eq!(exec_errors.len(), 1);
        match &exec_errors[0].code {
